...
def B_outside_sphere(sphere_radius, dipole_pos, dipole_moment, detector_positions):
    ...
    assert np.all(
        np.linalg.norm(detector_positions, axis=1) > sphere_radius
    ), "All points must be outside the sphere"

    assert np.all(
        np.linalg.norm(dipole_pos) < sphere_radius
...
def tms_E_field(dipole_pos, dipole_moment, didt, positions):
    ...
    if dipole_pos.shape != dipole_moment.shape:
        raise ValueError(
            "List of dipole position and moments should have the same" "lengths"
        )
    mu0_4pi = 1e-7
